...
def createProblem(parameters): ###
    ...
    pType = parameters.get('pType')
    fileName = parameters.get('pFileName')
    
    if pType == 1:
        p = Numeric()
        p.setAlpha(parameters.get('alpha'))
        p.setDelta(parameters.get('delta'))
        p.setDx(parameters.get('dx'))
        p.setAType(parameters.get('aType'))
        
        p.createProblem(fileName)
    elif pType == 2:
        p = Tsp()
        p.setAlpha(parameters.get('alpha'))
        p.setDelta(parameters.get('delta'))
...
def createOptimizer(parameters): ###
    ...
    alg = eval(Optimizers[parameters.get('aType')])
    alg.setDelta(parameters.get('delta'))
    
    if parameters.get('aType') <= 4:
        alg.setNumRestart(parameters.get('numRestart'))
        
        if parameters.get('aType') == 4:
            alg.setDx(parameters.get('dx'))
            alg.setAlpha(parameters.get('alpha'))
        elif parameters.get('aType') == 2 or 3:
            alg.setLimitStuck(parameters.get('limitStuck'))
            
    elif parameters.get('aType') == 5:
        alg.setLimitEval(parameters.get('limitEval'))
        
    alg.setAType(parameters.get('aType'))
    alg.setPType(parameters.get('pType'))
    alg.setNumExp(parameters.get('numExp'))
